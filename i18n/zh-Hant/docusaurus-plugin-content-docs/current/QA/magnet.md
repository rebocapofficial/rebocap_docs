...
```mind:height=350,width:100%,title=抗磁模式選擇,color
- **磁場環境**
	- **所有磁場環境一致性好**
		- **跳舞**
			- 關閉抗磁，抗磁強度10以上，不要使用6軸
		- **慢節奏舞蹈**
			- 使用抗磁，抗磁強度12，不要使用6軸
		- **普通社交、睡覺**
			- 使用抗磁，抗磁強度12，不要使用6軸
	- **部分區域存在干擾，如地板**
		- **跳舞**
			- 關閉抗磁，抗磁強度10以上，不要使用6軸（部分節點在某些位置可能會歪）
			- 使用抗磁，抗磁強度12，不要使用6軸（無法連續長時間使用，使用1~3分鐘建議靜止幾秒）
			- 6軸模式，使用一段時間以後，需要重新校準
		- **慢節奏舞蹈**
			- 使用抗磁，抗磁強度12，不要使用6軸
		- **普通社交、睡覺**
			- 使用抗磁，抗磁強度12，不要使用6軸
	- **磁場環境基本不可用（如彈簧床、或者房間周圍有變壓器）**
		...
		- **跳舞、慢節奏舞蹈**
			- 6軸，使用一段時間以後重新動作校準
		- **正常社交**
			- 減少走動和其它運動，一般能夠堅持更久再校準
```


```mind:height=350,width:100%,title=how to choose magnet mode,color
...
```mind:height=350,width:100%,title=耐磁模式的選擇,color
- **磁場環境**
    - **一致的磁場環境**
        - **舞蹈**
            - 抗磁模式關閉，磁力10以上，6軸請勿使用。
        - **慢舞**
            - 使用抗磁模式，磁力12，6軸請勿使用。
        - **正常社交與睡眠**
            - 使用抗磁模式，磁力12，6軸請勿使用。
    - **某些區域有干擾（例如：彈簧床）**
        - **舞蹈**
            - 抗磁模式關閉，磁力10以上（某些節點可能在特定位置移動）。
            - 使用抗磁模式，磁力12（不適合長時間使用，建議每1〜3分鐘休息幾秒鐘）。
            - 6軸模式：長時間使用後需要重新校準。
        - **慢舞**
            - 使用抗磁模式，磁力12，6軸請勿使用。
        - **正常社交與睡眠**
            - 使用抗磁模式，磁力12，6軸請勿使用。
    - **磁場環境有限（例如：彈簧床或變壓器附近的房間）**
        - **睡眠**
            - 使用抗磁模式，然後使用6軸。
        - **舞蹈與慢舞**
            - 6軸模式：長時間使用後需要重新校準。
        - **正常社交**
            - 減少移動或其他活動，通常可以延遲重新校準。
```
