...
```mind:height=350,width:100%,title=抗磁モード選択,color
- **磁場環境**
	- **すべての磁場環境が一貫している**
		- **ダンス**
			- 抗磁モードをオフにし、磁場強度を10以上に設定し、6軸を使用しない。
		- **スローペースのダンス**
			- 抗磁モードを使用し、磁場強度を12に設定し、6軸を使用しない。
		- **通常の社交、睡眠**
			- 抗磁モードを使用し、磁場強度を12に設定し、6軸を使用しない。
	- **一部のエリアに干渉がある場合（例：床）**
		- **ダンス**
			- 抗磁モードをオフにし、磁場強度を10以上に設定し、6軸を使用しない（特定の位置で一部のノードがずれる可能性あり）。
			- 抗磁モードを使用し、磁場強度を12に設定し、6軸を使用しない（連続使用には不向き、1〜3分使用後に数秒間静止を推奨）。
			- 6軸モード：長時間使用後に再校正が必要。
		- **スローペースのダンス**
			- 抗磁モードを使用し、磁場強度を12に設定し、6軸を使用しない。
		- **通常の社交、睡眠**
			- 抗磁モードを使用し、磁場強度を12に設定し、6軸を使用しない。
	- **磁場環境がほとんど使用できない場合（例：スプリングベッド、またはトランスフォーマーの近くの部屋）**
		- **睡眠**
			- 抗磁モードを使用し、その後6軸を使用。
		- **ダンス、スローペースのダンス**
			- 6軸モード：長時間使用後に再校正が必要。
		- **通常の社交**
			- 移動や他の活動を減らし、再校正を通常より遅らせることが可能。
```
...
```
```mind:height=350,width:100%,title=耐磁モードの選択,color
- **磁場環境**
    - **一貫した磁場環境**
        - **ダンス**
            - 抗磁モードをオフにし、磁力10以上、6軸は使用しないでください。
        - **ゆっくりしたダンス**
            - 抗磁モードを使用し、磁力12、6軸は使用しないでください。
        - **通常の社交と睡眠**
            - 抗磁モードを使用し、磁力12、6軸は使用しないでください。
    - **一部のエリアで干渉あり（例：スプリングベッド）**
        - **ダンス**
            - 抗磁モードをオフにし、磁力10以上（一部のノードは特定の位置でシフトする可能性があります）。
            - 抗磁モードを使用し、磁力12（連続使用には適さないため、1〜3分ごとに数秒休憩を推奨）。
            - 6軸モード：長時間使用後に再キャリブレーションが必要です。
        - **ゆっくりしたダンス**
            - 抗磁モードを使用し、磁力12、6軸は使用しないでください。
        - **通常の社交と睡眠**
            - 抗磁モードを使用し、磁力12、6軸は使用しないでください。
    - **磁場環境が限られている（例：スプリングベッドまたは変圧器の近くの部屋）**
        - **睡眠**
            - 抗磁モード、その後6軸を使用してください。
        - **ダンスとゆっくりしたダンス**
            - 6軸モード：長時間使用後に再キャリブレーションが必要です。
        - **通常の社交**
            - 移動やその他の活動を減らし、通常は再キャリブレーションを遅らせることができます。
```
